...
def main():
    ...
    dataset = load_dataset('text', data_files={'train': 'zul.test.txt'})
    file = open("surface_segments_test.txt", "a", encoding="utf-8")
    for index in range(dataset['train'].num_rows):
        batch = tokenizer.tokenize(dataset['train'][index]['text'])
        tmp = []
        ...
        for word, label in zip(batch, labels):
            ...
            del labels
            del tmp
            print(f"Epoch done: {str(index)}")
    print("CRF Segmentation Completed")
    print("Starting ULM training...")
    spm.SentencePieceTrainer.train(input='drive/MyDrive/segments/surface_segments_train.txt', model_prefix='morph_final', vocab_size=10000, model_type='unigram')
    print("ULM training done.")
main()